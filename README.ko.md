...
가민 및 애플 워치의 골프 라운드 데이터를 분석하는 데스크탑 앱입니다. Tauri 2(하이브리드 프레임워크), 러스트, 바닐라 자바스크립트로 제작되었습니다.

🇺🇸 [English README](README.md)

## 개요

가민 골프 워치(어프로치, FR9xx, Epix, ...)는 라운드당 두 개의 FIT 파일에 골프 활동 데이터를 저장합니다:
...
![코스 통계](images/ko/coursestats1.png)
![코스 통계 — 클럽 요약](images/ko/coursestats2.png)
...
첫 실행 시 이름과 기기 유형(가민 워치 또는 애플 워치)을 입력하는 설정 모달이 표시됩니다. 이 설정은 `save_settings`를 통해 저장되며 표시되는 동기화 UI를 제어합니다 — 애플 워치 선택 시 "워치 동기화" 버튼이 숨겨지고 로컬 WiFi(mDNS)를 통해 자동으로 동기화됩니다.
...
| `get_settings` | 저장된 `Settings` 객체 반환 (`player_name`, `device_source`) |
| `save_settings` | `Settings`를 스토어에 저장 |
...
- `source` — `"garmin"` 또는 `"apple"` (기존 라운드는 기본값 `"garmin"`)
